...
namespace NativeTrees
{
        ...
        where T : unmanaged
    {
        ...
        public int Capacity => math.max(nodes.Capacity, objects.Capacity);
        public int MaxDepth => maxDepth;
        public int ObjectsPerNode => objectsPerNode;
        ...
        public NativeOctree(AABB bounds, int objectsPerNode, int maxDepth, Allocator allocator, int initialCapacity = 0)
        {
            if (maxDepth <= 1 || maxDepth > MaxDepthLimit)
                throw new ArgumentOutOfRangeException(nameof(maxDepth), "Max depth is " + MaxDepthLimit);
            if (!bounds.IsValid)
        ...
        /// </summary>
        public const int MaxDepthLimit = 8 * sizeof(int) / 3;
